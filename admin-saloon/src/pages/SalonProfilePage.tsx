...
export default function SalonProfilePage({ user, onLogout }: Props) {
  ...
  const [form, setForm] = useState({ name: '', city: '', starting_price: '', latitude: '', longitude: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.city || !form.starting_price) return alert('Name, city, and starting price are required.');
    ...
    <Layout user={user?.email || 'Admin'} onLogout={onLogout}>
      <div className="page-root container">
        <div className="page-header">
          <div>
            <h1>Salon Profile</h1>
            <div className="subtitle">Manage your salon's public information</div>
          </div>
          {!isEditing && (
            <button className="btn-add" onClick={() => setIsEditing(true)}>Edit Profile</button>
          )}
        </div>
